...
class ActivityForm(forms.ModelForm):
    ...
    name = forms.CharField(
        label="Nombre de la actividad", widget=forms.TextInput(attrs={"class": "input"}), required=True
    )

    description = forms.CharField(
        label="Descripción", widget=forms.Textarea(attrs={"class": "textarea"}), required=True
    )

    equipment = forms.ModelChoiceField(
        Equipment.objects.all(), required=False, label="Equipamiento", widget=forms.Select(attrs={"class": "control"})
    )

    space_1 = forms.ModelChoiceField(
        Space.objects.all(), required=False, label="Espacio 1", widget=forms.Select(attrs={"class": "control"})
    )

    space_2 = forms.ModelChoiceField(
        Space.objects.all(), required=False, label="Espacio 2", widget=forms.Select(attrs={"class": "control"})
    )

    space_3 = forms.ModelChoiceField(
        Space.objects.all(), required=False, label="Espacio 3", widget=forms.Select(attrs={"class": "control"})
    )

    date_start = forms.DateTimeField(
        input_formats=["%d/%m/%Y %H:%M"],
        label="Fecha de inicio",
        widget=forms.DateInput(attrs={"class": "input", "placeholder": "dd/mm/aaaa hh:mm"}),
        required=True,
    )
    ...
    admin_link = forms.URLField(
        ...
        widget=forms.TextInput(
            attrs={"class": "input", "placeholder": "Este link es necesario para algunas áreas (detalladas más abajo)."}
        ),
        required=False,
    )
...
class NotificationForm(forms.Form):
    ...
    subject = forms.CharField(label="Título del correo", widget=forms.TextInput(attrs={"class": "input"}))
    body = forms.CharField(label="Cuerpo", widget=forms.Textarea(attrs={"class": "textarea"}))

    @staticmethod
    def send_email(subject, body, mailer, emailed):
        html_content = render_to_string("emails/activity_status.html", {"body": body})
        text_content = strip_tags(html_content)

        send_mail(subject, text_content, mailer, emailed, fail_silently=False, html_message=html_content)
